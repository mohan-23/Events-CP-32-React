...
class Events extends Component {
  state = {eventsLists: eventsList, eventStatus: ''}
  ...
  render() {
    const {eventsLists, eventStatus} = this.state
    // console.log(eventStatus)

    return (
      <div className="event-registration-cart">
        <div className="event-container">
          <h1 className="event-heading">Events</h1>
          <ul className="event-item-container">
            {eventsLists.map(eachEvent => (
              <EventItem
                eventDetails={eachEvent}
                key={eachEvent.id}
                onStatusEvent={this.onStatusEvent}
              />
            ))}
          </ul>
        </div>
